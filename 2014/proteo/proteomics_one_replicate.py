...
df2=df2[(df2[['Sample1_01']].min(axis=1)>0) | (df2[['Sample1_02']].min(axis=1)>0)]
df2['Sample1']=df2[['Sample1_01']].mean(axis=1)
df2['Sample2']=df2[['Sample1_02']].mean(axis=1)
...
fig.savefig(table1[:-4]+'-one-rep-log2.png',dpi=300,bbox_inches='tight')
plt.close(fig)
...
plt.xlabel('Log2 Spectrum Count Replicate 1')
plt.ylabel('Log2 Spectrum Count Replicate 2')
ax1.set_xlim([-0.5,max([max(df2['Sample1-log2']),max(df2['Sample2-log2'])])])
ax1.set_ylim([-0.5,max([max(df2['Sample1-log2']),max(df2['Sample2-log2'])])])
fig.savefig(table1+'-one-rep-log2-zeros.png',dpi=300,bbox_inches='tight')
plt.close(fig)
...
plt.xlabel('Log2 Spectrum Count Ratio (Replicate 2/Replicate 1)')
plt.ylabel('Log2 Sum Spectrum Count')
ax1.set_xlim([-10.5,10.5])
ax1.set_ylim([min(df2['log2_sum']),max(df2['log2_sum'])])
fig.savefig(table1+'-one-rep-ratio-sum.png',dpi=300,bbox_inches='tight')
plt.close(fig)
...
ax1.set_xlim([-10.5,10.5])
fig.savefig(table1+'-one-rep-log2-hist.png',dpi=300,bbox_inches='tight')
plt.close(fig)

df2.to_csv(table1[:-4]+'-one-rep.protein.txt',index=True,index_label='protein',sep='\t',na_rep='')